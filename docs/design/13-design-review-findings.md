...
- **i18nキー化未実装** — バックエンド（`User.locale`・ロケールキー型YAML）の布石はあるが、
  ...
  少ないと判断し保留。
- **共有視聴ページにリボン（読み取り専用）が無い（11章:123）** — 仕様上の曖昧さがあり保留。
  共有ページが再生するのは既に「編集済み（デッドタイム除去済み）」の動画のため、通常の
  試合詳細ページのリボン（プレー区間/デッドタイムの可視化）をそのまま持ち込む意味が無い
  （編集済み動画は定義上「全部プレー」）。ハイライト位置を示すリボンなのか、原本タイムライン
  との対応を示すものなのか、意図の確認が必要。プロダクト側の意図を確認してから実装すべき。
- **スイング一覧ストリップ未実装（12章:193）** — フォーム解析詳細ページの新規UIコンポーネント
  （有効/無効・外れ値スイングの可視化、タップで該当値表示）。今回のスコープでは着手せず。
