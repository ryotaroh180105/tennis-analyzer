# 12a. shot-mechanics.v1.yaml 文献レビュー：調査結果（打ち切り・ユーザー承認済み）

タスク#26の調査結果。**4指標はshot-mechanics.v1.yamlに反映済み**（下表）。
citation_statusは全体としてはまだ「検証中」のまま（理由は下記）。

**2026-07-11、ユーザー判断でこれ以上の本文PDF入手は追わないことに決定**。
このセッションからは学術ドメインへの到達手段（curl・WebFetchとも）が無く、
本文確認には時間がかかりすぎるため。以下は打ち切り時点の状態のまま。
backhand/smash/volleyは未反映・citation_statusは「検証中」で据え置き。
再開する場合は本ファイル末尾の「次のアクション」から。
