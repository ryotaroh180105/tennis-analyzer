...
足すのは数ヶ月仕事）。防御の構造は [09](09-go-to-market.md) の moat 定義に従う：
時間限定の先行の間に、規模に依らない非対称資産（大会・コーチの関係資産、日本環境の
正解データ）を確保する。AdviceLog は競技・言語を越えて移転可能な学習資産だが
規模依存であり、単独では防御にならないことを自覚して設計する。
