...
import {
  api,
  SERVE_METRIC_LABEL_JA,
  SERVE_METRIC_STATUS_LABEL_JA,
  SERVE_PHASE_LABEL_JA,
  SERVE_STATUS_LABEL_JA,
  type ServeMetric,
} from "@/lib/api";

const STATUS_COLOR: Record<ServeMetric["status"], string> = {
  in_range: "var(--court)",
  borderline: "var(--sand)",
...
function formatValue(m: ServeMetric): string {
  if (m.measured == null) return "—";
  const unit = m.unit === "deg" ? "°" : m.unit === "ms" ? "ms" : "";
  return `${m.measured}${unit}`;
}

function formatRange(m: ServeMetric): string {
  const unit = m.unit === "deg" ? "°" : m.unit === "ms" ? "ms" : "";
  return `参考: ${m.elite_range[0]}〜${m.elite_range[1]}${unit}`;
}

export default function ServeDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();

  const { data: session } = useSWR(["serve-session", id], () => api.getServeSession(id), {
    refreshInterval: (data) => (data && ["done", "failed"].includes(data.status) ? 0 : 3000),
  });
  const isDone = session?.status === "done";
  const { data: analysis } = useSWR(isDone ? ["serve-analysis", id] : null, () => api.getServeAnalysis(id));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      {!isDone && session.status !== "failed" && (
        ...
        >
          {SERVE_STATUS_LABEL_JA[session.status]}
        </div>
      )}
      ...
      {isDone && analysis && (
        <div style={{ padding: "8px 16px 40px" }}>
          ...
          >
            ...
            （骨格検出率: {Math.round(analysis.confidence.pose_detection_ratio * 100)}%）。
                ...
                {analysis.feedback_metrics.map((mid) => {
                  ...
                  return (
                    <div key={mid} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--surface-raised)", border: "1px solid var(--line-hair)", borderRadius: 10, padding: "8px 12px" }}>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 700 }}>{SERVE_METRIC_LABEL_JA[mid] || mid}</div>
                      ...
                      <div style={{ textAlign: "right" }}>
                        ...
                        <div style={{ fontSize: 10, color: STATUS_COLOR[m.status] }}>{SERVE_METRIC_STATUS_LABEL_JA[m.status]}</div>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {analysis.metrics.map((m) => (
              <div key={m.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "6px 0", borderBottom: "1px solid var(--line-hair)" }}>
                <span>
                  {SERVE_METRIC_LABEL_JA[m.id] || m.id}
                  <span style={{ color: "var(--ink-secondary)", fontSize: 10, marginLeft: 6 }}>
                    ({SERVE_PHASE_LABEL_JA[m.phase] || m.phase})
                  </span>
                </span>
